...
const Home = () => {
  ...
  const [booksMercadoLibre, setBooksMercadoLibre] = useState<Book[] | null>(
    null
  );
  ...
  const getBook = async (isbn: string) => {
    ...
    const res = await fetch(
      `https://search-book-buscalibre-function-vwp6agkcra-uc.a.run.app?isbn=${isbn}`
    );
    ...
    const resAntartica = await fetch(
      `https://search-book-antartica-function-vwp6agkcra-uc.a.run.app?isbn=${isbn}`
    );
    const dataAntartica = await resAntartica.json();
    setBookAntartica(dataAntartica.data);

    const resMercadoLibre = await fetch(
